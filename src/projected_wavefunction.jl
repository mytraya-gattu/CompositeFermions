...
- `jk_type::Int64`: Jastrow power LLL-projected with each orbital, so `Q₁ = jk_type·(N-1)/2`
  and the ESP roots are taken with multiplicity `jk_type`. `jk_type = 1` is the standard
  JK projection. Independent of `p`.
...
- `reg_coeffs::Vector{Float64}`: ESP normalization, `reg_coeffs[i] = i/(jk_type·(N-1)-i+1)`
  so `∏ = 1/C(jk_type·(N-1), k)`.
...
per-sector `Qstars` and `l_m_lists`). The JK projection uses `jk_type = 1` for every sector
(per-sector `jk_type` is not well defined); the ESP roots have multiplicity 1.
...
"""
    Ψproj(Qstar, p, system_size, l_m_list; jk_type::Int64 = 1)
...
`jk_type` (≥ 1) sets the Jastrow power carried through the LLL projection
(`Q₁ = jk_type·(N-1)/2`, ESP root multiplicity `jk_type`); `jk_type = 1` is the standard
projection and reproduces the previous behaviour exactly.
"""
function Ψproj(Qstar::Rational{Int64}, p::Int64, system_size::Int64, l_m_list::Vector{NTuple{2,Rational{Int64}}}; jk_type::Int64 = 1)
    ...
    for (Liter, L) in enumerate(Lgrid)
        ...
        fourier_matrix[liters[Liter], begin:begin+numerator(L-Qstar), numerator(1-L+Lmax):1:numerator(1+L+Lmax)] .= generate_fourier_matrices(Qstar, system_size, L, last.(l_m_list[liters[Liter]]); jk_type=jk_type)
    ...
    for i in eachindex(reg_coeffs)
        reg_coeffs[i] = i / (jk_type * (system_size - 1) - i + 1)
    end

    return Ψproj(Qstar, p, jk_type, system_size, l_m_list, Lmax, μ_list, Lz_list, fourier_tot_matrix, U, V, exp_θ, exp_ϕ, dist_matrix, u_v_ratio_matrix, elementary_symmetric_polynomials, reg_coeffs, wigner_d_matrices, wigner_D_matrices, jastrow_factor_log, slater_det)
end
...
the effective monopole strength and occupied orbitals of parton `i`. Uses `jk_type = 1`.
"""
...
# --- ESP order / multiplicity per wavefunction type (shared update code below) ---

_esp_order(Ψ::Ψproj) = numerator(Ψ.Lmax - Ψ.Qstar)
_esp_order(Ψ::Ψparton) = Ψ.max_JK
_esp_mult(Ψ::Ψproj) = Ψ.jk_type
_esp_mult(Ψ::Ψparton) = 1
...
function update_wavefunction!(Ψ::Union{Ψproj,Ψparton}, θ::Vector{Float64}, ϕ::Vector{Float64})
    ...
    @simd for electron_iter in axes(Ψ.elementary_symmetric_polynomials, 2)
        @inbounds @views get_symmetric_polynomials!(Ψ.elementary_symmetric_polynomials[:, electron_iter], Ψ.u_v_ratio_matrix[:, electron_iter], b, Ψ.reg_coeffs; mult=m)
    end
