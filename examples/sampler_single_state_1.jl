...
include("../src/monte_carlo_utilities.jl")
...
function gibbs_thermalization!(RNG, Ψcurrent::T, Ψnext::T, θcurrent::Vector{Float64}, ϕcurrent::Vector{Float64}, θnext::Vector{Float64}, ϕnext::Vector{Float64}, σinit::Float64, logpdf::Function, num_thermalization::Int64, Q::Rational{Int64}) where {T <: Union{Ψproj, Ψparton, ΨoneLL}}

    acceptance_target::Float64 = 0.5 ### Gibbs sampling.
    a::Float64, b::Float64 = MonteCarloOnSphere.arm_parameters(acceptance_target, 3.0)
    ...
    update_wavefunction!(Ψcurrent, vcat(θcurrent[begin:(end - 1)], [0.0]), vcat(ϕcurrent[begin:(end - 1)], [0.0]))
    copy!(Ψnext, Ψcurrent)

    logpdf_current = logpdf(Ψcurrent) + 4.0 * Q * log(cos(θcurrent[end] / 2.0))
    ...
    for monte_carlo_iter in 1:num_thermalization
        ...
        θnext[sampling_iter], ϕnext[sampling_iter] = MonteCarloOnSphere.proposal(RNG, θcurrent[sampling_iter], ϕcurrent[sampling_iter], σ)

        if sampling_iter != length(θcurrent)
            update_wavefunction!(Ψnext, θnext[sampling_iter], ϕnext[sampling_iter], sampling_iter)
        end

        logpdf_next = logpdf(Ψnext) + 4.0 * Q * log(cos(θnext[end] / 2.0))
        ...
        if monte_carlo_iter ∈ tuning_schedule
            ...
            δ = MonteCarloOnSphere.arm_scale_factor(num_samples_accepted_thermalization / monte_carlo_iter, acceptance_target, a, b)
            σ *= δ
        end
...
function gibbs_sampler(filename::String, Qstar::Rational{Int64}, l_m_list::Vector{NTuple{2, Rational{Int64}}}, p::Int64, num_thermalization::Int64 = 5 * 10^5, num_steps::Int64 = 10^6)
    ...
    Ψcurrent = Ψproj(Qstar, 2 * p, system_size, l_m_list)
    Ψnext = Ψproj(Qstar, 2 * p, system_size, l_m_list)
    ...
    θcurrent, ϕcurrent = MonteCarloOnSphere.rand_θ_ϕ_gen(RNG, system_size)

    # θcurrent[end] = 0.0 ### North pole.
    # ϕcurrent[end] = 0.0 ### North pole.
    ...
    sampling_iter, σ, δt_therm, thermalization_acceptance_rate = gibbs_thermalization!(RNG, Ψcurrent, Ψnext, θcurrent, ϕcurrent, θnext, ϕnext, σ, logpdf, num_thermalization, Q)
    ...
    for monte_carlo_iter in 1:num_steps
        ...
        MonteCarloOnSphere.update_density!(θmesh, θcurrent, accumulated_density)
...
sample_cf_gs("./data_1/", parse(Int64, ARGS[1]), 4, 1, 1)
