...
let
    ...
    for mult in (1, 2, 3)
        ...
        write_cmat(joinpath(OUT, "esp_mult$(mult).csv"), reshape(dest, :, 1))
    end
end
...
# --- 3. PsiProj slater_det (jk_type 1 and 2) and PsiUnproj slater_det ------------------
function dump_proj(tag, N, n, p, jk_type)
    Qstar = (N // n - n) // 2
    ...
    ψ = Ψproj(Qstar, p, Nsys, l_m_list; jk_type=jk_type)
...
dump_proj("proj1", 10, 2, 1, 1)
dump_proj("proj2", 10, 2, 2, 2)
